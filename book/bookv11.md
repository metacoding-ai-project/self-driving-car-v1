...
2단계: 전부 더하고, 편향(bias)도 더한다
   ...
   왜 전부 더할까?
   → 11개 상태값이 각각 "이 상황이 좋은지 나쁜지"를
     가중치를 곱해서 점수로 변환한 것!
   → 전부 더하면 = "종합 점수"가 됨!
   → 벽이 있는 곳(②, ⑥)의 점수가 높게 나왔으니
     이 뉴런은 "벽 근처 상황"에 민감하게 반응하는 뉴런!
...
3단계: 활성화 함수를 통과시킨다 (ReLU)
   ...
   2.86의 의미:
   → 이 뉴런은 현재 상황에서 "2.86만큼 반응했다"는 뜻!
   → 이 숫자 자체가 직접 "직진해!" 같은 답은 아님!
   → 64개 뉴런이 각각 다른 반응값을 내놓고,
     그것들이 모여서 최종 Q값이 됨!
```
...
#### 🎯 그런데 뉴런 하나에서 2.86이 나왔는데, Q값 3개는 어떻게 만들어지나요?

이 부분이 가장 헷갈리는 포인트입니다! 비유로 쉽게 설명하겠습니다.

```
[뉴런 1개 → Q값 3개가 만들어지는 과정]

비유: 요리 대회!

은닉층 64명의 요리사가 각각 재료(11개 상태값)를 받아서
자기만의 소스를 하나씩 만듭니다.

  요리사 1번: 소스 = 2.86 (벽 감지 전문)
  요리사 2번: 소스 = 0.42 (방향 감지 전문)
  요리사 3번: 소스 = 1.73 (거리 감지 전문)
  ...
  요리사 64번: 소스 = 0.91

이제 출력층에 "심사위원 3명"이 있습니다!
  심사위원 A (직진 담당): 64개 소스를 자기만의 비율로 섞음
    → Q(직진)   = 8.5
  심사위원 B (우회전 담당): 64개 소스를 다른 비율로 섞음
    → Q(우회전) = 3.2
  심사위원 C (좌회전 담당): 64개 소스를 또 다른 비율로 섞음
    → Q(좌회전) = 2.1

즉!
  은닉층 64개 뉴런 → 각각 숫자 1개씩 출력 (총 64개)
  출력층 3개 뉴런 → 64개 숫자를 각자 다르게 조합 → Q값 3개!
```
...
> 💡 **어렵게 느껴지면 이것만 기억하세요!**
> - 뉴런은 **"입력(상태값) × 가중치를 더하는 계산기"**
> - 가중치는 **"이 정보가 얼마나 중요한지를 나타내는 숫자"** (처음엔 랜덤, 학습하며 수정!)
> - 은닉층 64개 뉴런이 각각 숫자를 내놓고, 출력층 3개 뉴런이 이걸 조합해서 **Q값 3개**를 만듦
