...
핵심은 **1보다 작은 수를 계속 곱하면 점점 작아진다**는 것입니다. 한 번 곱하면 티도 안 나지만, 수백 번 곱하면 엄청난 차이가 생깁니다!

```
비유: 용돈 10,000원에 매일 0.995를 곱한다면?
...
→ 0.5%씩 줄어드는 건 티가 안 나지만,
  그게 100번 쌓이면 40%가 사라지는 것!
```

이걸 탐험 확률에 적용하면 0.995와 0.998의 차이가 확 보입니다:
