...
**중요 개념**: 모든 버전(v1~v5)에서 에이전트는 목적지의 **절대 좌표를 모릅니다**. 대신 **상대적 방향**만 알고 움직입니다.
...
## 챕터 5: v5 - 추론 속도 최적화와 캐싱 시스템

### 📖 챕터 개괄

v5에서는 **추론(Inference) 시 실행 속도를 최적화**합니다!

**학습 목표:**
- 추론 시 캐싱으로 속도 향상 방법 이해
- 정적 환경에서 캐시 활용
- 동적 환경에서 Policy 우선 원칙 습득
- 강화학습 철학 깨달음
...
### 🔍 핵심 철학 2: 추론이 잘 작동하면 캐싱으로 빠르게!

**V5의 핵심 아이디어:**

```
학습 완료 후, 추론(Inference) 시:
→ 같은 맵에서 반복 실행하면
→ 효율적인 경로를 발견함
→ 이 경로를 캐싱하면 빠른 실행 가능!
```
...
→ Policy 계산(45ms)을 Cache 조회(12ms)로 대체!
→ 약 3.75배 빠름!
```
...
V5의 핵심은 **추론(Inference) 시 캐싱으로 빠른 실행**을 달성하는 것입니다.
...
- 학습은 신경망을 변화시키는 과정
- 캐시는 경험 메모일 뿐, 학습과 무관
- 다양한 탐험이 필요하므로 캐시 불필요

#### 2단계: 실행 (main.py without Cache)
**목표: 추론 시 Policy로 경로 발견**
...
**결과:**
```
Step 1-10: 여러 경로 시도 (Policy가 매번 계산, 45ms)
Step 11-20: 효율적 경로 발견 시작
Step 21-50: 최적 경로 확립 (여전히 매번 Policy 계산, 45ms)
```

**개선 가능성 발견:**
- 💡 같은 맵에서 같은 경로를 반복 실행
- 💡 매번 Policy Network 계산 (45ms 소요)
- 💡 이미 잘 작동하는 경로인데 계속 계산
- 💡 **이 경로를 캐싱하면 빠르지 않을까?**
...
# 실행 시 Policy 계산 후 캐시 참고
action = agent.select_action(state, training=False)
# → Policy 계산 (안전성)
# → Cache 참고 (일치 확인)
# → 실전에서는 Cache 우선 확인으로 최적화 가능
```

**효과:**
```
Step 1-10: 캐시 없음, Policy 계산 (45ms)
Step 11-20: 일부 캐시 저장 (경험 축적)
Step 21-50: 대부분 캐시 히트 (빠름)
Step 51+: 거의 모든 캐시 히트 (최고 속도)

실전 최적화 시:
→ 캐시 히트: 12ms (3.75배 빠름!)
→ 캐시 미스: 45ms (Policy 계산)
```
...
```python
# 정적 환경: 장애물이 안 바뀜
env = GridEnvironment(random_map=False, enable_dynamic_obstacles=False)

for step in range(100):
    action = agent.select_action(state)  # 캐시 활용
    # ✅ 잘 작동! 빠름!
```

**캐싱의 한계를 보여주는 예시 (동적 환경):**
```python
# 동적 환경: 장애물이 계속 생김
...
for step in range(51, 100):
    # 만약 캐시만 믿으면?
    cached_action = cache.get(state)  # 옛날 경험
    # ❌ 새 장애물을 모르므로 충돌 가능!

    # Policy는?
    policy_action = policy_net(state)  # 현재 상태 반영
    # ✅ 새 장애물을 감지하고 회피!
```
...
### 🔧 v5의 동적 장애물 시스템 (캐싱 한계 테스트용)

**목적:**
```
캐싱의 한계를 시연하고
Policy > Cache 원칙의 필요성을 보여주기 위한 테스트 시스템
```
...
### ⚠️ 왜 현재 v5 코드는 느리게 설계되었나요?

**중요한 교육적 선택!**

현재 v5 코드를 실행하면 캐싱을 사용해도 **속도 향상이 거의 없습니다**. 왜일까요?

#### 현재 코드 구조 (교육용)

```python
def select_action(self, state, training=False):
    # 1️⃣ Policy를 항상 먼저 계산 (45ms)
    policy_action = self.policy_net(state).argmax()

    # 2️⃣ 그 다음 캐시 확인 (1ms)
    cached_action = self.cache.get(state)

    # 3️⃣ 비교만 하고 Policy 반환
    return policy_action  # Policy 우선!
```

**결과:** Policy 계산(45ms) + Cache 확인(1ms) = **항상 45ms 소요**

#### 왜 이렇게 만들었나요?

**교육 목적: 정적 환경과 동적 환경을 모두 이해하기 위해!**

v5는 두 가지 환경 전략을 **보여주기 위한** 코드입니다:

**1. 정적 환경 전략 (Static Environment)**
```python
# 실무 버전 1: 캐시 우선
if cache.has(state):
    return cache.get(state)  # 12ms (3.75배 빠름!) ✅
return compute_policy(state)  # 45ms
```

**효과:**
- 장애물이 변하지 않는 환경
- 캐시 히트 시 12ms (Policy 45ms보다 3.75배 빠름)
- 성능 최적화 우선

**2. 동적 환경 전략 (Dynamic Environment)**
```python
# 실무 버전 2: 안전 우선
policy_action = compute_policy(state)  # 45ms
return policy_action  # Cache 무시, 안전 우선! ✅
```

**효과:**
- 장애물이 계속 변하는 환경
- 항상 Policy 계산 (최신 상태 반영)
- 안전성 우선

#### 환경별 전략 요약

| 환경 타입 | 우선순위 | 평균 응답 시간 | 특징 |
|----------|---------|--------------|------|
| **정적 환경** | Cache 먼저 | 12ms | 속도 최적화 |
| **동적 환경** | Policy 먼저 | 45ms | 안전성 우선 |
| **현재 v5 (교육용)** | 항상 Policy | 45ms | 두 전략 이해 |

#### 💡 핵심 이해

```
현재 v5 = 교육용 "느린" 설계
목적: Policy > Cache 원칙과 환경별 전략을 이해하기 위해

실무 적용 시:
→ 정적 환경: Cache 먼저 확인 (빠름)
→ 동적 환경: Policy 먼저 계산 (안전)
```

**비유:**
```
교육용 v5 = 운전 학원
- 항상 강사가 옆에서 확인 (Policy 계산)
- 안전하지만 느림
- 두 가지 운전 방법을 모두 배움
...
- 같은 맵에서 여러 번 실행
- 캐시가 쌓임 (경험 축적)
- 현재 v5는 교육용이므로 속도 향상은 제한적
- 실전에서는 캐시 우선 확인으로 3.75배 빠름 가능

#### Step 3: 동적 장애물 테스트 (캐싱 한계 시연)

`config.py` 수정:

```python
USE_CACHE = True
ENABLE_DYNAMIC_OBSTACLES = True  # 동적 장애물 활성화 (테스트용)
```
...
**관찰:**
- 50스텝마다 오렌지색 장애물 생성 (캐싱 한계 시연)
- Policy가 실시간으로 장애물 감지 및 회피
- 캐시만 믿으면 충돌했을 상황을 Policy가 대응
- Policy > Cache 원칙의 필요성 확인
...
- [ ] 학습의 진짜 목적을 이해했다
- [ ] 추론 시 캐싱으로 속도 최적화하는 방법을 알았다
- [ ] 캐싱과 학습의 차이를 알았다
- [ ] 정적 환경에서 캐싱의 효과를 이해했다
- [ ] 동적 환경에서 Policy 우선 원칙을 깨달았다
...
5. 추론 최적화 (Inference Optimization)
   → 정적 환경: 캐싱 활용 (빠름)
   → 동적 환경: Policy 우선 (안전)

6. 올바른 이해 (Understanding)
   → Policy > Cache 원칙
```
