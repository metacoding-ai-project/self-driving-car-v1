...
>
> 한 줄의 코드도 모르던 메타코딩이 5주 만에 자율주행 AI를 만들기까지,
> 실패와 성장의 여정을 따라가다 보면 어느새 강화학습의 핵심이 손에 잡힙니다.
...
- [프롤로그: 운명을 바꾼 월요일 아침](#프롤로그-운명을-바꾼-월요일-아침)
- [0화: if문의 벽 (v0)](#0화-if문의-벽-v0)
- [1화: AI에게 눈을 달아주다 (v1)](#1화-ai에게-눈을-달아주다-v1)
- [2화: 넘어져야 걷는 법을 안다 (v2)](#2화-넘어져야-걷는-법을-안다-v2)
- [3화: 가짜 90점의 진실 (v3)](#3화-가짜-90점의-진실-v3)
- [4화: 처음엔 크게, 나중엔 섬세하게 (v4)](#4화-처음엔-크게-나중엔-섬세하게-v4)
- [5화: 현장에서 살아남기 (v5)](#5화-현장에서-살아남기-v5)
- [번외편: 망치 너머의 도구들 (v6)](#번외편-망치-너머의-도구들-v6)
- [에필로그: 실패는 다음 버전의 씨앗이다](#에필로그-실패는-다음-버전의-씨앗이다)
...
여기서 **"얼마나 좋을까?"를 숫자로 나타낸 것**이 바로 **Q값(Q-value)** 입니다.
...
처음에는 이 Q값이 엉터리입니다. 아직 아무것도 모르니까요. 하지만 수백 번 직접 해보면서 "이 상황에서 직진하니까 벽이었어 → Q값을 낮추자", "이 상황에서 우회전하니까 목적지에 가까워졌어 → Q값을 올리자"를 반복하면, Q값이 점점 정확한 리뷰 점수가 됩니다.
...
> 💡 **ReLU는 뭐고, Sigmoid는?**
> 방금 3단계에서 본 **ReLU**는 "음수는 0으로 차단, 양수는 통과"하는 필터입니다. 쓸모없는 신호(음수)를 걸러내서 중요한 정보만 남기는 역할이에요. 프롤로그에서 배운 **Sigmoid**는 숫자를 0~1 사이 확률로 바꾸는 함수였죠? 우리 AI의 은닉층에서는 **ReLU**를 씁니다. Sigmoid는 "스팸일 확률 92%"처럼 확률이 필요할 때 사용합니다.
...
학습 과정에서 **ε(엡실론) = 0.79** 같은 숫자를 봤죠? 이건 "79% 확률로 랜덤하게 행동"한다는 뜻입니다. AI에게는 두 가지 모드가 있습니다:
...
역전파가 "결과에서 거꾸로 돌아가면서 각 가중치를 조정한다"는 건 이해했죠? 그런데 **"각 가중치를 얼마만큼 조정해야 하는 걸까?"** 이걸 계산하는 공식이 **연쇄법칙**입니다.
...
핵심: 결과에서 가까울수록 영향력이 크다!
  ...
  지유(b): 현우를 거침 → 0.3 × 0.5 = 0.15 (줄어듦 → 조금 수정)
  상민이(a): 둘 다 거침 → 0.4 × 0.3 × 0.5 = 0.06 (아주 작음 → 살짝 수정)
...
메타코딩의 가슴이 철렁 내려앉았습니다. 1화에서 발견한 **과적합**이 또 나타난 것입니다!
...
"메타코딩 씨, **DQN만이 정답은 아니에요.** 같은 문제를 완전히 다른 방식으로 풀 수도 있어요."
...
1주차 (프롤로그 + 0화: if문의 벽 + 1화: AI에게 눈을 달아주다):
