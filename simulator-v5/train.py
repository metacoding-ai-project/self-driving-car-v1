...
from config import (CURRENT_SPEED, NUM_EPISODES, SHOW_TRAINING, NUM_MAPS, MAPS_PER_EPISODE,
                   RANDOM_SEED, USE_CACHE, ENABLE_DYNAMIC_OBSTACLES)
...
def train():
    # 초기화
    pygame.init()
    env = GridEnvironment(random_map=True, enable_dynamic_obstacles=ENABLE_DYNAMIC_OBSTACLES)  # v5: 동적 장애물 옵션 추가
    agent = DQNAgent(use_cache=USE_CACHE)  # v5: 캐싱 옵션 추가
    ...
    print("=" * 60)
    print("🚗 자율주행 강화학습 시뮬레이터 v5 - Policy > Cache")
    print("=" * 60)
    print(f"총 에피소드: {num_episodes}")
    print(f"맵 개수: {NUM_MAPS}개")
    print(f"맵당 에피소드: 약 {MAPS_PER_EPISODE}개")
    print(f"캐싱 시스템: {'활성화' if USE_CACHE else '비활성화'}")
    print(f"동적 장애물: {'활성화' if ENABLE_DYNAMIC_OBSTACLES else '비활성화'}")
    print("훈련을 시작합니다!")
    print("학습 목적: 최적 경로가 아닌 환경 적응력 획득")
    if ENABLE_DYNAMIC_OBSTACLES:
        print("⚠️  동적 장애물이 활성화되어 있습니다 - Policy > Cache 원칙 확인!")
    ...
    for episode in range(num_episodes):
        ...
        while not done and running:
            ...
            # Learning Rate Scheduling: 에피소드마다 학습률 업데이트
            current_lr = agent.update_learning_rate(episode, num_episodes)
            ...
            if SHOW_TRAINING:
                ...
                texts = [
                    ...
                    f"Epsilon: {agent.epsilon:.3f}  |  LR: {agent.learning_rate:.6f}  |  Avg Reward: {avg_reward:.1f}",
