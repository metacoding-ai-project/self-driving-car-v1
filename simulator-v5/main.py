...
from config import USE_CACHE, ENABLE_DYNAMIC_OBSTACLES

def test():
    # 초기화
    # 테스트셋: 훈련에 사용하지 않은 새로운 맵으로 테스트
    # random_map=True: 완전히 새로운 랜덤 맵 생성 (과적합 검증)
    env = GridEnvironment(random_map=True, enable_dynamic_obstacles=ENABLE_DYNAMIC_OBSTACLES)  # v5: 동적 장애물 옵션
    start_x, start_y = env.start_pos
    car = Car(start_x, start_y)
    agent = DQNAgent(use_cache=USE_CACHE)  # v5: 캐싱 옵션
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r:
                    # R키: 새로운 테스트 맵으로 리셋 (훈련에 사용하지 않은 맵)
                    env = GridEnvironment(random_map=True, enable_dynamic_obstacles=ENABLE_DYNAMIC_OBSTACLES)  # v5: 동적 장애물 옵션
                    start_x, start_y = env.start_pos
                    car.reset(start_x, start_y)
                    state = env.get_state(car.x, car.y, car.direction)
                    print(f"🔄 리셋! 새로운 테스트 맵 (랜덤 생성)")
                    print(f"   시작점: {env.start_pos}, 목적지: {env.goal_pos}")
                    print(f"   ⚠️  이 맵은 훈련에 사용하지 않은 새로운 맵입니다!")
        ...
        if done:
            ...
            # 새로운 테스트 맵으로 리셋 (훈련에 사용하지 않은 맵)
            env = GridEnvironment(random_map=True)  # 새로운 테스트 맵 생성
            start_x, start_y = env.start_pos
            car.reset(start_x, start_y)
            state = env.get_state(car.x, car.y, car.direction)
        else:
